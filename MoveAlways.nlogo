...
agents-own [
  ...
  legitimacy          ; How legit do they think the government is
]
...
to setup
  ...
  create-agents round (initial-agent-density * .01 * count patches) [
    ...
    set legitimacy random-normal 0.5 0.2
    set active? false
    set jail-term 0
    display-agent
  ]

  ; create agents with guns
  create-guns round (initial-gun-density * .01 * count patches) [
    move-to one-of patches with [ not any? turtles-here ]
    display-guns
  ]
...
to determine-behavior
    ...
    becomeActive
    set activeagentscounter activeagentscounter + 1  ]
end

to totalagentscounter
    if (grievance - risk-aversion * estimated-arrest-probability > threshold) [
    becomeActive
    set howmanyactive howmanyactive + 1
    set activeagentscounter activeagentscounter + 1 ]
end
...
to-report estimated-arrest-probability
  ...
  let g count guns-on neighborhood
  let a 1 + count (agents-on neighborhood) with [ active? ]
  ; See Info tab for a discussion of the following formula
  report 1 - exp (- k * floor (( c + g ) / a))
end
to neighborhoodcount
  if active? [
  set neighbourhood neighbourhood + ( count (agents-on neighborhood) with [ active? ] )
  ]
end
...
to enforce
  if any? (agents-on neighborhood) with [ active? ] [
    ...
    ask suspect [
      ...
      set jailedagents jailedagents + 1
    ]
  ]
end
...
to radius enforce
  if any? (agents-on neighborhood) with [ active? ] [
    ...
    ask suspect [
      set active? false
      set jail-term random jail-time-intimidation
    ]
  ]
end
...
@#$#@#$#@
<experiments>
  <experiment name="intimidatie altijd 2 40" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    ...
    <enumeratedValueSet variable="initial-gun-density">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="jail-time-intimidation">
      <value value="40"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="intimidatie altijd 2 45" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 2 50" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 2 55" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 2 60" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 4 60" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 4 55" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 4 50" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 4 45" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="intimidatie altijd 4 40" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count agents with [ active? ]</metric>
  ...
  <experiment name="MoveAlways" repetitions="16" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="250"/>
    <metric>activeagentscounter</metric>
    <metric>jailedagents</metric>
    <metric>howmanyactive</metric>
    <metric>neighbourhood</metric>
    <enumeratedValueSet variable="jail-time-intimidation">
      <value value="40"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="movement?">
